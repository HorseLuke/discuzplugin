...
/*
RatelogVIEWER Main PHP File 1 For Discuz 6.1.0/6.1.0F Build 20080918
...
if ($action == 'myrate' || $action == 'berated'){
	...
	if($action == 'myrate'){
	    ...
	    $ratelogcount = $db->result_first("SELECT COUNT(*) FROM {$tablepre}ratelog rl WHERE rl.uid=$uid AND rl.extcredits IN ($ids) $filtersql $datelinesql");
	    $ratelogcount =  empty($ratelogcount) ? 0 : $ratelogcount;
    }else{
		$navtitle=$username.'的被评记录';
    	$ratelogcount = $db->result_first("SELECT COUNT(*) FROM {$tablepre}ratelog rl LEFT JOIN {$tablepre}posts p ON rl.pid=p.pid WHERE p.authorid=$uid AND rl.extcredits IN ($ids) $filtersql $datelinesql");
	    $ratelogcount =  empty($ratelogcount) ? 0 : $ratelogcount;
	}
