...
class DiscuzUserController extends BaseController{
    ...
    public function actionGetPostlist(){
        ...
        if($result['totalCount'] > 0){
            $multipage = multi($result['totalCount'], $limitnum, $this->_param['page'] , "userpostlist.php?action=getPostlist&amp;uid={$targetUser->uid}&amp;handlekey=iirs_userPostList_{$targetUser->uid}");
        }
    ...
    public function actionGetThreadlist(){
        ...
        if($result['totalCount'] > 0){
            $multipage = multi($result['totalCount'], $limitnum, $this->_param['page'] , "userpostlist.php?action=getThreadlist&amp;uid={$targetUser->uid}&amp;handlekey=iirs_userPostList_{$targetUser->uid}");
