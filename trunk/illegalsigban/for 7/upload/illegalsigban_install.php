...
$installuser='Install_illegalsigban';
$thisfilename='illegalsigban_install.php';
$thisfileauthor='Horse Luke(竹节虚)[2008]';
$thisfileversion='0.0.1 build 20081226';
$title='屏蔽违规签名安装程序';
...
./illegalsigban.php
./APACHELICENSE2.txt
./templates/default/illegalsigban_admin.htm
./templates/default/illegalsigban_ban.htm
./templates/default/illegalsigban_sidebar.htm
EOT;

$installSQL = <<<EOT
DROP TABLE IF EXISTS `{$tablepre}illegalsigbanlog`;
CREATE TABLE `{$tablepre}illegalsigbanlog` (                         
                `logid` int(10) unsigned NOT NULL AUTO_INCREMENT,
                `banstatus` tinyint(1) NOT NULL DEFAULT '1',
                `banuid` mediumint(8) unsigned NOT NULL DEFAULT '0',
                `operateruid` mediumint(8) unsigned NOT NULL DEFAULT '0',
				`operater` char(15) NOT NULL,
                `bandatetime` int(10) unsigned NOT NULL DEFAULT '0',
                `unbaninfo` varchar(50) NOT NULL,
                `banreason` varchar(255) NOT NULL,
                PRIMARY KEY (logid),
                INDEX banuid(banuid,banstatus)
) ENGINE=MyISAM DEFAULT CHARSET=gbk;
DELETE FROM `{$tablepre}caches` WHERE cachename = 'cacheillegalsigbanuidlist' LIMIT 1;
INSERT INTO `{$tablepre}caches` (`cachename` ,`type` ,`dateline` ,`expiration` ,`data` )VALUES ('cacheillegalsigbanuidlist', '1', '$timestamp', '0', '0');
EOT;
	...
	function checkok() {
      ...
      <tr>
        ...
        '.$prevbutton.$nextbutton.$exittoindexbutton.'
	      ...
	      $db->query("DROP TABLE IF EXISTS `{$tablepre}illegalsigbanlog`;");
		  $db->query("CREATE TABLE `{$tablepre}illegalsigbanlog` (                         
                `logid` int(10) unsigned NOT NULL AUTO_INCREMENT,
                `banstatus` tinyint(1) NOT NULL DEFAULT '1',
                `banuid` mediumint(8) unsigned NOT NULL DEFAULT '0',
                `operateruid` mediumint(8) unsigned NOT NULL DEFAULT '0',
	      ...
	      $db->query("DELETE FROM `{$tablepre}caches` WHERE cachename = 'cacheillegalsigbanuidlist' LIMIT 1;");
		  $db->query("INSERT INTO `{$tablepre}caches` (`cachename` ,`type` ,`dateline` ,`expiration` ,`data` )VALUES ('cacheillegalsigbanuidlist', '1', '$timestamp', '0', '0');");				 
	  ...
	  if ($step==1){     //第1步,确认执行SQL语句.
          templates("header",'卸载步骤一:确认执行SQL语句');
		  echo'
		      <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td class="content">
      ...
      <tr>
        <td class="content">
		<textarea name="notice" style="width: 98%; height: 50px" readonly>'.$uninstallSQL.'</textarea>
		<br /><br />注意：卸载数据库完毕后，记得删除如下插件文件：<br />
        <textarea name="notice" style="width: 98%; height: 100px" readonly>'.$fileList.'</textarea>
