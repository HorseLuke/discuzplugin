...
if(empty($action) || $action == 'admin') {
	...
	if(submitcheck('bansubmit')) {
		 ...
		 if($banstatusnew==1) {
			 ...
			 $banreasonnew=empty($banreasonnew) ? '' : '屏蔽操作理由：'.$banreasonnew;
			 if($sendreasonpm=='1'){
			       $subject='[系统消息]您的个人签名因涉嫌违规而被管理员屏蔽，请尽快修改。';
				   $message='[系统消息]您的个人签名因涉嫌违规而被管理员屏蔽。请仔细阅读社区规章，并尽快修改个人签名。'.$banreasonnew; 
