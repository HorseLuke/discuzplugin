...
if($action == 'alllogs') {
    ...
    if($op=='basicsettings'){
	    if(!submitcheck('basicsettingssubmit')) {	
		    ...
		    $medaleasybuy_basicsettings['open']=intval($opennew);
			$medaleasybuy_basicsettings['buyextcreditsid']=intval($medalcanbuyextcreditsidnew);
		    ...
		    showmessage('基本设置成功！准备刷新缓存......', 'medaleasybuyadmincp.php?action=set&op=refreshcache');
		}
	    ...
	    if ($set=='list'){
		    ...
		    if(!submitcheck('addmedalsubmit')) {
			     ...
			     showmessage('添加勋章到可购买列表成功！准备刷新缓存......', 'medaleasybuyadmincp.php?action=set&op=refreshcache');
			  ...
			  if ($delmedal){
				 ...
				 showmessage('删除成功！准备刷新缓存......', 'medaleasybuyadmincp.php?action=set&op=refreshcache'); 
			  }
			  $moneyamountnew = $moneyamountnew ? intval($moneyamountnew) : 0;
			  $db->query("UPDATE {$tablepre}medaleasybuymedals SET moneyamount='$moneyamountnew' WHERE medalid='$medalid'");
			  showmessage('修改成功！准备刷新缓存......', 'medaleasybuyadmincp.php?action=set&op=refreshcache');
	        ...
	        $medaleasybuy_basicsettings = $db->result_first("SELECT data FROM {$tablepre}caches WHERE cachename='medaleasybuy_basicsettings'");
			if (empty($medaleasybuy_basicsettings)){
                   $medaleasybuy_basicsettings=array(
						     'open'=> 0,
							 'buyextcreditsid'=> 2,
							 );
                   $db->query("REPLACE INTO `{$tablepre}caches` (`cachename` ,`type` ,`dateline` ,`expiration` ,`data` )VALUES ('medaleasybuy_basicsettings', '1', '$timestamp', '0', '\$medaleasybuy_basicsettings=".daddslashes(var_export($medaleasybuy_basicsettings,true))."');");
                   showmessage('缓存出现错误并修复成功，准备重新刷新......', 'medaleasybuyadmincp.php?action=set&op=refreshcache');
			}
