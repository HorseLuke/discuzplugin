 ...
 * @version $Id: Error.php 111 2010-02-24 06:07:04Z horseluke@126.com $
...
class Inter_Error
{
    ...
    public static function error_handler($errno, $errstr, $errfile, $errline) {
    	...
    	//dz要这样设置才不会有这么多错误显示.....（开始）
    	//error_reporting(E_ERROR | E_WARNING | E_PARSE);
        $PermitOutput = true;
        $DZPermitOutputErrno = array("1"=>"E_ERROR",
                                    "2"=>"E_WARNING",
                                    "4"=>"E_PARSE",
                                    "16"=>"E_CORE_ERROR",
                                    "32"=>"E_CORE_WARNING",
        ...
        if(!array_key_exists($errno,$DZPermitOutputErrno)){
            $PermitOutput = false;
	    return '';
        }
        //dz要这样设置才不会有这么多错误显示.....（结束）
            ...
            foreach (self::$_allError as $key => $errorInfo){
                ...
                if('detail' == self::$conf['logType'] && !empty($errorInfo['trace'])){
                    ...
                    foreach ( $errorInfo['trace'] as $stack => $trace ){
                        $logText .= $prefix. $stack. "\t". implode("\t", $trace). "\n";
                    }
                }
    ...
    public static function error_display(){
        if(false != self::$conf['debugMode'] && !empty(self::$_allError) ){
                ...
                if(empty($errorInfo['trace'])){
                    ...
                    foreach ($errorInfo['trace'] as $stack => $trace){
                        $htmlText .= '<tr>
										...
										<td>'.$trace['line'].'</td>
										<td>'.$trace['class']. htmlspecialchars($trace['function']) .'</td>
									</tr>';
                    }
                    $htmlText .= '</table>';
                }
            ...
            $variables_content = '';
            //DZ的
            foreach(array("_GET", "_POST", "_DSESSION", "_DCOOKIE", "_SESSION", "_COOKIE") as $key){
                $variables_link .= '<a href="#variables'.$key.'">$'.$key.'</a>&nbsp;';
                $variables_content .= '<div class="intererrorsubtitle"><a name="variables'.$key.'" id="variables'.$key.'"></a>$'.$key.'</strong></div>
										  <div class="intererrorcontent">';
...
<div class="intererrorblock">
    <div class="intererrorsubtitle">Variables: {$variables_link}</div>
    {$variables_content}
</div>
END;
}
